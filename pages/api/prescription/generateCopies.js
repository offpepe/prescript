...
const generateCopies = async (_req, res) => {
    try {
        const src = fs.readFileSync('prescription.pdf');
        ...
        doc.pipe(fs.createWriteStream('prescriptionCopie.pdf'));
        ...
        res.status(500).json({ err: { code: 'internal_erorr', message: error.message }});
    }
}
